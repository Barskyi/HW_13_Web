...
access_to_route_all = RoleAccess([Role.admin, Role.moderator])  # , Role.user
...
async def get_todo(todo_id: int, db: AsyncSession = Depends(get_db),
                   user: User = Depends(auth_service.get_current_user)):  # todo_id: int = Path(ge=1)
...
@router.put('/{todo_id}')
async def update_todo(body: TodoUpdateSchema, todo_id: int, db: AsyncSession = Depends(get_db),
                      user: User = Depends(auth_service.get_current_user)):
